...
class ActionDatasetAttention(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(start, (start+self.interval*self.max_len)):
            if (i - start) % self.interval == 0:
                if self.mode == "image":
                    ...
                    pos2list = [item for pair in zip(loaded_arrays['x_list'], loaded_arrays['y_list']) for item in pair]
                    pos2list = np.array(pos2list) / float(self.img_size)
        ...
        del image, trainImages, pos2Images
